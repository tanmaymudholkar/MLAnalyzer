...
for iEvt in range(iEvtStart,iEvtEnd):
    ...
    #if len(rhTree.SC_dvtx) == 0: continue # branch can be empty if pguns fail isolation

    idx = [rhTree.runId, rhTree.lumiId, rhTree.eventId]

    EB_time = np.array(rhTree.EB_time).reshape(1,170,360)
    ...
    for i in range(nPhoEvt):
        ...
        if rhTree.SC_recoIdx[i] == -1: continue
        #assert rhTree.SC_recoIdx[i] != -1
        ...
        data['boost'] = rhTree.SC_boost[i]
        data['dvtx'] = rhTree.SC_dvtx[i]
        data['cTau'] = data['dvtx']/data['boost']

        data['vtx'] = rhTree.SC_vtx[i]
        data['vtxT'] = rhTree.SC_vtxT[i]
        data['vtxZ'] = rhTree.SC_vtxZ[i]
        data['cTauExt'] = data['vtx']/data['boost']

        if data['ieta'] < 15 or data['ieta'] >= 170-16:
            continue
        ...
        #print(EB_time.shape)
        #print(data['iphi'], data['ieta'])
        time = crop_EBshower(EB_time, data['iphi'], data['ieta'])
        ...
        #print(data['Xtz'].shape)
        #print(time.shape)
        data['Xtztime'] = np.concatenate([time, data['Xtz']], axis=0)
