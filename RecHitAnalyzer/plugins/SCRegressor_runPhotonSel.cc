...
void SCRegressor::branchesPhotonSel ( TTree* tree, edm::Service<TFileService> &fs )
{
  ...
  tree->Branch("SC_gen_photon_projEE_X", &vSC_gen_photon_projEE_X_);
  tree->Branch("SC_gen_photon_projEE_Y", &vSC_gen_photon_projEE_Y_);
}
...
bool SCRegressor::runPhotonSel ( const edm::Event& iEvent, const edm::EventSetup& iSetup ) {
  ...
  for ( auto& iG : vGenPhoIdxs ) {
    ...
    bool passes_preselection = SCRegressorHelpers::passesTriggerInspiredPreselection(iRecoPho);
    if (!passes_preselection) continue;
    // if ( std::fabs(iRecoPho->pt()) <= ptCut ) continue;
    // if ( std::fabs(iRecoPho->eta()) >= etaCut ) continue;
...
void SCRegressor::fillPhotonSel ( const edm::Event& iEvent, const edm::EventSetup& iSetup )
{
  ...
  vSC_genY_.clear();
  vSC_genZ_.clear();
  vSC_gen_photon_projEE_X_.clear();
  vSC_gen_photon_projEE_Y_.clear();
  for ( auto const& iPho : vPhos ) {
    ...
    std::tuple<float, float> xy_gen_photon = SCRegressorHelpers::get_xy_at_given_z_from_eta_phi(distance_from_plane, std::fabs(iGen->eta()), iGen->phi());
    vSC_gen_photon_projEE_X_.push_back(std::get<0>(xy_gen_photon));
    vSC_gen_photon_projEE_Y_.push_back(std::get<1>(xy_gen_photon));
