...
void RecHitAnalyzer::branchesTRKlayersAtECALadjustable ( TTree* tree, edm::Service<TFileService> &fs ) {
  ...
  //TOB
      ...
      sprintf(hname, "evt_TOB_layer%d_ECALAdj%s",layer,zside);
  ...
  //TEC
	 for ( int iL(0); iL < nTEC; iL++ ) {
	    ...
	    hTEC_ECALadj[iL][proj] = fs->make<TH2F>(hname, htitle,
		  ...
		  sprintf(hname, "evt_TEC_layer%d_ECALAdj%s",layer,zside);
	  ...
	  for ( int iL(0); iL < nTIB; iL++ ) {
	    ...
	    hTIB_ECALadj[iL][proj] = fs->make<TH2F>(hname, htitle,
		  ...
		  sprintf(hname, "evt_TIB_layer%d_ECALAdj%s",layer,zside);
  ...
  //TID
	  for ( int iL(0); iL < nTID; iL++ ) {
	    ...
	    hTID_ECALadj[iL][proj] = fs->make<TH2F>(hname, htitle,
		  ...
		  sprintf(hname, "evt_TID_layer%d_ECALAdj%s",layer,zside);
	  ...
	  for ( int iL(0); iL < nBPIX; iL++ ) {
	    ...
	    hBPIX_ECALadj[iL][proj] = fs->make<TH2F>(hname, htitle,
		  ...
		  sprintf(hname, "evt_BPIX_layer%d_ECALAdj%s",layer,zside);
  ...
  //FPIX
	  for ( int iL(0); iL < nFPIX; iL++ ) {
	    ...
	    hFPIX_ECALadj[iL][proj] = fs->make<TH2F>(hname, htitle,
		  ...
		  sprintf(hname, "evt_FPIX_layer%d_ECALAdj%s",layer,zside);
...
void RecHitAnalyzer::fillTRKlayerHelper (int layer_, unsigned int proj, TH2F *hSUBDET_ECAL[][Nadjproj], TH2F *hEvt_Adj_SUBDET[][Nadjproj], const CaloGeometry* caloGeom, const float& eta, const float& phi)
{
  ...
  if ( ecalId.subdetId() == EcalBarrel )
  {
    ...
    fillByBinNumber(hSUBDET_ECAL[layer_][proj], phi_eta, 1.0);
    fillByBinNumber(hEvt_Adj_SUBDET[layer_][proj], phi_eta, 1.0);
  }
  else if ( ecalId.subdetId() == EcalEndcap )
  {
    ...
    hSUBDET_ECAL[layer_][proj]->Fill( phi2, eta );
    hEvt_Adj_SUBDET[layer_][proj]->Fill( phi2, eta);
  }
}
    ...
    {//loop on the rechit
      if (pixeliter->isValid())
      {
        ...
        if ( subid == PixelSubdetector::PixelBarrel ){
          fillTRKlayerHelper (layer, proj, hBPIX_ECAL, hEvt_Adj_BPIX, caloGeom, eta, phi);
        }
        else if ( subid == PixelSubdetector::PixelEndcap ){
          fillTRKlayerHelper (layer, proj, hFPIX_ECAL, hEvt_Adj_FPIX, caloGeom, eta, phi);
        }
      }
    }
  }
     ...
     for(;stripiter!=stripRechitRangeIteratorEnd;++stripiter)
      {
        if (stripiter->isValid())
        {
          ...
          if ( subid == StripSubdetector::TOB ) {
            fillTRKlayerHelper (layer, proj, hTOB_ECAL, hEvt_Adj_TOB, caloGeom, eta, phi);
          }
          else if ( subid == StripSubdetector::TEC ) {
            fillTRKlayerHelper (layer, proj, hTEC_ECAL, hEvt_Adj_TEC, caloGeom, eta, phi);
          }
          else if ( subid == StripSubdetector::TIB ) {
            fillTRKlayerHelper (layer, proj, hTIB_ECAL, hEvt_Adj_TIB, caloGeom, eta, phi);
          }
          else if ( subid == StripSubdetector::TID ) {
            fillTRKlayerHelper (layer, proj, hTID_ECAL, hEvt_Adj_TID, caloGeom, eta, phi);
          }
        }
      }
    }
